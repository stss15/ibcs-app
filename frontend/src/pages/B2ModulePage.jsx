    ...
    <article className={cn("b2-card", { "is-success": status === "success", "is-error": status === "error" })}>
      ...
      <div className="b2-segment-nav">
        ...
        <div className="b2-segment-nav__actions">
          ...
          <button type="button" className="btn btn--primary" onClick={handleSubmit} disabled={resolved}>
            Check answers
          </button>
        </div>
      </div>
...
function OrderingActivity({ segment, onComplete, onBack, isTeacher, attemptStats, onAttempt }) {
  ...
  const [status, setStatus] = useState("idle");
  const [resolved, setResolved] = useState(false);
  const numbers = useMemo(() => items.map((_, index) => String(index + 1)), [items]);
  ...
  function handleSubmit() {
    ...
    const correct = items.every((item, index) => order[index] === String(item.correctIndex + 1));
      ...
      >
        {segment.questions?.map((question) => (
          <CheckpointQuestion
            ...
            showRationale={locked && isCorrect(question, responses[question.id])}
            disabled={locked}
...
function isCorrect(question, value) {
  ...
  switch (question.type) {
    ...
    case "multi-select": {
      ...
      return sortedExpected.every((entry, index) => entry === sortedValue[index]);
    }
    default:
      return false;
  }
                  ...
                  <input
                    ...
                    onChange={(event) => {
                      ...
                      if (checked) {
                        ...
                        next = next.filter((entry) => entry !== option.id);
                      }
                      onChange(next);
                    }}
                  />
